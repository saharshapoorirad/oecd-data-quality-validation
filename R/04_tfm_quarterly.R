# =============================================================================
# TFM_master_quarterly_final_CORRECTED_FULL.R
#
# Quarterly non-financial Transaction-Flow Matrix (TFM) builder
...
#
# NOTE:
#   The Stage-1 GG sheet may contain explicitly documented S1-residual
#   derivations when direct quarterly S13 rows are unavailable. See the
#   Stage-1 QA_GG_source_status and QA_GG_derivation sheets.
#
# =============================================================================
...
exact_input <- file.path(
  ...
  paste0(
    country_code,
    "_stage1_nonfinancial_accounts_quarterly_output.xlsx"
  )
)
...
input_pattern <- paste0(
  "^",
  country_code,
  "_stage1_nonfinancial_accounts_quarterly_output(\\([0-9]+\\))?\\.xlsx$"
)
...
find_input_file <- function(exact_name, pattern, search_dir) {
  ...
  if (length(hits) == 0) {
    stop(
      "Stage-1 quarterly output not found in:\n",
...
output_file <- file.path(
  output_dir,
  paste0(country_code, "_stage2_TFM_quarterly_output.xlsx")
)

message("Using input file: ", input_file)
message("Output file: ", output_file)
message("Standalone corrected quarterly Stage-2 TFM script.")
...
read_stage1_sheet <- function(file, sheet) {
  ...
  period_names <- c("Quarter", "Period", "Year", "period")
  ...
  for (i in seq_len(nrow(preview))) {
    ...
    if (any(vals %in% period_names, na.rm = TRUE)) {
      header_row <- i
      break
    }
  }
  ...
  if (is.na(header_row)) {
    stop(
      "Sheet ", sheet,
      " does not contain a Quarter/Period/Year header in the first 20 rows."
    )
  }
  ...
  period_col <- intersect(period_names, names(x))

  if (length(period_col) == 0) {
    stop(
      "Sheet ", sheet,
      " header was detected at row ", header_row,
      " but no usable period column remained after reading."
    )
  }

  names(x)[names(x) == period_col[1]] <- "Period"

  x$Period <- stringr::str_squish(as.character(x$Period))
  x <- x[!is.na(x$Period) & x$Period != "", , drop = FALSE]

  x
}

period_order <- function(x) {
  x <- as.character(x)

  year <- suppressWarnings(
    as.integer(sub("^([0-9]{4}).*$", "\\1", x))
  )

  quarter <- suppressWarnings(
    as.integer(sub("^.*Q([1-4]).*$", "\\1", x))
  )

  ifelse(
    is.na(year),
    NA_integer_,
    year * 4L + ifelse(is.na(quarter), 0L, quarter)
  )
}
...
expected_inputs <- tribble(
  ...
  "HH",  "P1",  "OUTh",          "Direct if available",
  ...
  "ROW", "D45", "RNTrR|RNTrS",   "Source net reversed to ROW perspective; validated against S1",
...
make_S2 <- function(d, domestic_b9) {
  ...
  out <- tibble(
    ...
    # Correct quarterly ROW D45 orientation.
...
domestic_b9 <- s11_wide %>%
  select(Period, S11_B9 = B9) %>%
  full_join(
    s12_wide %>% select(Period, S12_B9 = B9),
    by = "Period"
  ) %>%
  full_join(
    s13_wide %>% select(Period, S13_B9 = B9),
    by = "Period"
  ) %>%
  full_join(
    hh_wide %>% select(Period, HH_B9 = B9),
    by = "Period"
  )
...
all_periods <- all_periods[
  order(period_order(all_periods), all_periods)
]

matrix_base <- row_dictionary %>%
  crossing(Period = all_periods) %>%
  left_join(s11_long, by = c("Period", "Code")) %>%
  left_join(s12_long, by = c("Period", "Code")) %>%
  left_join(s13_long, by = c("Period", "Code")) %>%
  left_join(hh_long,  by = c("Period", "Code")) %>%
  left_join(s2_long,  by = c("Period", "Code"))
...
matrix_complete <- matrix_base %>%
  ...
  ungroup() %>%
  mutate(
    Period_order = period_order(Period)
  ) %>%
  arrange(
    Period_order,
    Row_order
...
latest_period_matrix <- matrix_complete %>%
  filter(
...
notes <- tribble(
  ...
  "Quarter format",
  "Period is read from Quarter/Period/Year labels and ordered chronologically with period_order().",

  "Stage-1 adjustment basis",
  "The final quarterly Stage-1 pipeline must use one OECD ADJUSTMENT basis consistently and must never sum N and Y variants together.",
  ...
  "ROW D45 orientation",
  "Quarterly ROW D45 is sign-reversed relative to the raw R-S net so the matrix is expressed from the ROW perspective. This is not a residual-balancing adjustment.",
  ...
  "Scope limitation",
  "This is the quarterly non-financial TFM construction relative to the Stage-1 transaction universe and available OECD sector data. Financial accounts, stocks, revaluations and stock-flow links are separate later stages."
)
...
message(
  "Complete quarterly non-financial TFM workbook written: ",
  output_file
)

message(
  "Latest quarter included in TFM_latest_quarter: ",
  latest_period
)
